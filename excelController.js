...
async function exportDonors(res) {
  ...
  const workbook = await excelService.buildWorkbook({
    sheetName: 'Donors',
    columns: [
  ...
  await excelService.sendWorkbook(res, workbook, `donors_export_${Date.now()}.xlsx`);
}
...
async function exportPatients(res) {
  ...
  await excelService.sendWorkbook(res, workbook, `patients_export_${Date.now()}.xlsx`);
}
...
async function exportRequests(res) {
  ...
  await excelService.sendWorkbook(res, workbook, `requests_export_${Date.now()}.xlsx`);
}
...
async function exportStock(res) {
  ...
  await excelService.sendWorkbook(res, workbook, `stock_export_${Date.now()}.xlsx`);
}
...
async function exportDonations(res) {
  ...
  await excelService.sendWorkbook(res, workbook, `donations_export_${Date.now()}.xlsx`);
}
